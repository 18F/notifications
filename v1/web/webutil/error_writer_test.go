...
var _ = Describe("ErrorWriter", func() {
	...
	write := func(w http.ResponseWriter, err error) {
		writer.Write(w, err, context)

	}
	It("returns a 422 when a client tries to register a critical notification without critical_notifications.write scope", func() {
		write(recorder, webutil.UAAScopesError{errors.New("UAA Scopes Error: Client does not have authority to register critical notifications.")})
	...
	It("returns a 502 when CloudController fails to respond", func() {
		write(recorder, services.CCDownError{errors.New("Bad things happened!")})
	...
	It("returns a 500 when there is a template create error", func() {
		write(recorder, webutil.TemplateCreateError{})
	...
	It("returns a 500 when there is a template update error", func() {
		write(recorder, models.TemplateUpdateError{errors.New("Failed to update Template in the database")})
		Expect(recorder.Code).To(Equal(http.StatusInternalServerError))
		Expect(recorder.Body).To(MatchJSON(`{
	...
	It("returns a 404 when the space cannot be found", func() {
		write(recorder, services.CCNotFoundError{errors.New("Space could not be found")})
	...
	It("returns a 404 when the space cannot be found", func() {
		write(recorder, cf.NotFoundError{Message: "Space could not be found"})
		Expect(recorder.Code).To(Equal(http.StatusNotFound))
		Expect(recorder.Body).To(MatchJSON(`{
	...
	It("returns a 400 when the request cannot be parsed due to syntatically invalid JSON", func() {
		write(recorder, webutil.ParseError{})
	...
	It("returns a 422 when the requests are not valid due to semantically invalid JSON", func() {
		write(recorder, webutil.ValidationError{errors.New("invalid json")})
		Expect(recorder.Code).To(Equal(422))
		Expect(recorder.Body).To(MatchJSON(`{
			"errors": ["invalid json"]	
		}`))
	...
	It("returns a 422 when trying to send a critical notification without correct scope", func() {
		write(recorder, webutil.NewCriticalNotificationError("raptors"))
		Expect(recorder.Code).To(Equal(422))
		Expect(recorder.Body).To(MatchJSON(`{
	...
	It("returns a 409 when there is a duplicate record", func() {
		write(recorder, models.DuplicateError{errors.New("duplicate record")})
	...
	It("returns a 404 when a record cannot be found", func() {
		write(recorder, models.NotFoundError{errors.New("not found")})
	...
	It("returns a 406 when a record cannot be found", func() {
		write(recorder, services.DefaultScopeError{})
	...
	It("returns a 422 when a template cannot be assigned", func() {
		write(recorder, collections.TemplateAssignmentError{errors.New("The template could not be assigned")})
		Expect(recorder.Code).To(Equal(422))
		Expect(recorder.Body).To(MatchJSON(`{
	...
	It("returns a 422 when a user token was expected but is not present", func() {
		write(recorder, webutil.MissingUserTokenError{errors.New("Missing user_id from token claims.")})
		Expect(recorder.Code).To(Equal(422))
		Expect(recorder.Body).To(MatchJSON(`{
	...
	It("returns a 500 for unknown errors", func() {
		write(recorder, errors.New("unknown error"))
		Expect(recorder.Code).To(Equal(http.StatusInternalServerError))
		Expect(recorder.Body).To(MatchJSON(`{
